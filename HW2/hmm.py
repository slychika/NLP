...
def parse(filename):
    ...
    for line in f:
        ...
        if not testing:
            tcorrect.append(l[0]) 
            tincorrect.append(l[1])
        else:
            fcorrect.append(l[0]) 
            fincorrect.append(l[1])
...
def main():
    ...
    for i in range (len(fcorrect)):
        ...
        if fincorrect[i] != corrected[i]:
            c += 1
        totalLetters +=1
    percentStillWrong = b / totalLetters
    percentCorrect = 1-percentStillWrong
    ...
    print("Final percent (still) wrong:",  percentStillWrong)
    print("Final percent correct:", percentCorrect)
